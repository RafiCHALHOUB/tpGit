...
class Node():
    ...
    def display_node(self):
        '''
        Returns all nodes with their value/depth
        '''
        retour = str(self)
        if self.right:
    ...
    def print_tree(self, depth=0):
        '''
        Prints node root with it's descendant nodes forming an hierarchical tree like aspect
        '''
        retour = str(self)
        if self.right:
            retour += "\n"
            for i in range(0, depth+1):
                retour += "\t"
            retour += self.right.print_tree(depth+1)
        if self.left:
            retour += "\n"
            for i in range(0, depth+1):
                retour += "\t"
            retour += self.left.print_tree(depth+1)
        return retour
